...
const OrderHistory = () => {
  ...
  const userConnected = JSON.parse(user).id
...
console.log(userConnected)
  const getOrderHistory = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5139/api/User/${userConnected}`);
                ...
                <div className="collapse-content">
                  <div className="overflow-x-auto">
                    <table className="table max-sm:table-xs table-pin-rows table-pin-cols">
                      ...
                      <tbody>
                        {orders.map((product, counter) => (
                          <tr className="text-accent-content" key={nanoid()}>
                            <th>{counter + 1}</th>
                            <th><img src={`http://localhost:5139/images/${product.product.imageUrl}`} alt="product image" className="w-10" /></th>
